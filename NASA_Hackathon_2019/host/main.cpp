...
int main( int argc, char* argv[] )
{
    ...
    cl_mem d_c;
    cl_mem device_initConditions; 
    
    int number_of_pariticles = 10;
    int * host_initConditions = (int *) malloc(sizeof(int)*number_of_pariticles*3);
    
    
    srand(time(NULL));
    
    
    for(int c = 0; c < number_of_pariticles*3; c+=3){
        
        host_initConditions[c]   = 10 + rand( ) % 460;
        host_initConditions[c+1] = 10 + rand( ) % 460;
        host_initConditions[c+2] = 10 + rand( ) % 460;
        
        printf("%d \n", host_initConditions[c]);
        printf("%d \n", host_initConditions[c+1]);
        printf("%d \n", host_initConditions[c+2]);
    }
 ...
 **/
    ...
    kernel = clCreateKernel(program, "Dust", &err);
 
    // Create the input and output arrays in device memory for our calculation
    d_a = clCreateBuffer(context, CL_MEM_READ_ONLY, bytes, NULL, NULL);
    d_b = clCreateBuffer(context, CL_MEM_READ_ONLY, bytes, NULL, NULL);
    d_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY, bytes, NULL, NULL);
    device_initConditions = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int)*number_of_pariticles*3, NULL, NULL);
    ...
    err = clEnqueueWriteBuffer(queue, device_initConditions, CL_TRUE, 0,
                                  sizeof(int)*number_of_pariticles*3, host_initConditions, 0, NULL, NULL);
    ...
    err  = clSetKernelArg(kernel, 4, sizeof(cl_mem), &device_initConditions);
    ...
    clReleaseMemObject(device_initConditions);
